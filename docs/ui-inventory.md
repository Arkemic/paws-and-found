...
| `Modal` | `components/ui/Modal.jsx` | Native `<dialog>`: focus trap, Escape, inert background. Parent owns `isOpen`. Carries `m-auto` deliberately — the browser centres a dialog with `margin: auto` and Tailwind's reset zeroes it, which pins the dialog to the top-left. Do not remove it. **Present in the page only while open** (like `PhotoLightbox`), and focus returns to whatever opened it. |
...
| `Sidebar` | `components/Sidebar.jsx` | Workspace navigation. `panel` (admin): a white panel from `lg`, a wrapping row below. `light` (customer and staff): links on the page ground with a hairline divider and optional count badges, and below `lg` one compact section menu. Counts come from `WorkspaceLayout`'s `loadCounts`. |
