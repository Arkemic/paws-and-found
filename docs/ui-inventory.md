...
| `PatternVeil` | `components/PatternVeil.jsx` | IMG-014 as a decorative layer behind a section's content: the community pattern, faded out with a mask. It has to be its own element — a mask applies to everything inside the element it sits on, so putting it on the section fades the section's own text. The parent must be `relative isolate`. `aria-hidden`, no pointer events. |
