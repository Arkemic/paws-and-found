...
| `Sidebar` | `components/Sidebar.jsx` | Workspace navigation. `panel` (staff, admin): a white panel from `lg`, a wrapping row below. `light` (customer): links on the page ground with a hairline divider and optional count badges, and below `lg` one compact section menu. Counts come from `WorkspaceLayout`'s `loadCounts`. |
