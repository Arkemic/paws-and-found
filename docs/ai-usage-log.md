...
| 44 | 2026-09-06 | Claude (Claude Code) | Replace "Show more" with pagination | The first approach kept fetching the whole result set and slicing it in the browser, only with page links instead of a growing count | **Moved the paging to the database.** Explore now sends `page` and `per_page`, and the API answers with `LIMIT`/`OFFSET` plus a `meta` block it already produced. Verified in the network tab: one request per page, and page 2 shares no report with page 1 | Slicing a full list is the same work the "Show more" button was already doing. The point of paginating is to stop fetching rows nobody will look at — page links over a full fetch would have satisfied the guide's wording and none of its purpose |
| 45 | 2026-09-06 | Claude (Claude Code) | Same task — should the map be paged too? | Paging the whole page, map included | **Left the map unpaged**, asking for one large page instead. A comment already in `ExplorePage` said so: "a pin is cheap and paging a map would be confusing" | A map showing 9 of 32 pins is not a smaller map, it is a wrong one — someone searching their barangay would conclude nothing had been reported there |
| 46 | 2026-09-06 | Claude (Claude Code) | Same task — three counts that quietly became wrong | Left `reports.length` in the results heading, the filter dialog's button and the map caption | **All three now read the API's `total`.** With a full list they meant "how many matched"; with a page they silently changed meaning to "nine", so the heading would have read "9 pets found" above a four-page pager | The bug pagination introduces is not in the pager, it is in every count that used to be the whole list. Worth recording because nothing errored — the numbers were just wrong |
